...
document.addEventListener('DOMContentLoaded', async () => {
  document.addEventListener('click', event => {
    ...
    if (container && !container.contains(event.target)) toggleProfileMenu(false);
  });
...
function updateNavAuthUI() {
  ...
  if (appState.user) {
    const username = escapeHtml(appState.user.username || 'User');
    const email = escapeHtml(appState.user.email || 'No email added');
    const initials = escapeHtml(getUserInitials(appState.user.username));
    container.innerHTML = `
      <div class="relative">
        <button
          id="profile-menu-btn"
          onclick="toggleProfileMenu()"
          class="w-8 h-8 rounded-full bg-[#18181b] text-white text-[11px] font-semibold flex items-center justify-center hover:bg-black transition-colors focus:outline-none focus:ring-2 focus:ring-[#d4d4d0] focus:ring-offset-1"
        ...
        <div
          id="profile-menu"
          class="hidden absolute right-0 top-10 w-64 bg-white border border-[#ecece9] rounded-xl shadow-lg p-1.5 z-50"
          role="menu"
        >
          <div class="px-3 py-2.5 border-b border-[#f0f0ef]">
            <p class="text-xs font-semibold text-[#18181b] truncate">${username}</p>
            <p class="text-[11px] text-[#9b9a97] truncate mt-0.5">${email}</p>
          </div>
          <button
            onclick="handleLogout()"
            class="w-full mt-1 px-3 py-2 text-left text-xs text-[#5a5855] hover:text-[#18181b] hover:bg-[#f7f6f3] rounded-lg transition-colors flex items-center gap-2"
            role="menuitem"
          >
            <svg class="w-3.5 h-3.5 text-[#787774]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
              <path d="M10 17l5-5-5-5"></path>
              <path d="M15 12H3"></path>
              <path d="M21 19V5a2 2 0 0 0-2-2h-4"></path>
            </svg>
            <span>Sign Out</span>
          </button>
        </div>
    ...
    if (item.type === 'youtube') {
      typeBadgeHtml = `
        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold tracking-wider uppercase bg-red-50 text-red-700 border border-red-200">
          <svg class="w-3.5 h-3.5 shrink-0" viewBox="0 0 24 24" role="img" aria-label="YouTube">
            <rect x="2" y="4.5" width="20" height="15" rx="4" fill="#ff0000"></rect>
            <path fill="#ffffff" d="M10 8.25v7.5L16 12z"></path>
          </svg>
          <span>YOUTUBE</span>
        </span>
      `;
      ...
      typeBadgeHtml = `
        <span class="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[10px] font-semibold tracking-wider uppercase bg-emerald-50 text-emerald-700 border border-emerald-200">
          <svg class="w-3 h-3 text-emerald-600" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
        ...
        <div>
          ...
          <div class="flex items-center justify-between gap-2 mb-2.5">
            ...
            <div class="flex items-center gap-1 shrink-0">
              ...
              ` : ''}
              <button type="button" onclick="event.stopPropagation(); openEditModal('${item._id}')" class="p-1 text-[#9b9a97] hover:text-[#18181b] rounded transition-colors" title="Edit Item">
                <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"></path>
                  <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"></path>
                </svg>
              </button>
              <button type="button" onclick="event.stopPropagation(); handleDeleteContent('${item._id}')" class="p-1 text-[#9b9a97] hover:text-red-600 rounded transition-colors" title="Delete Item">
                <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                  <polyline points="3 6 5 6 21 6"></polyline>
                  <path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path>
                </svg>
          ...
          ${item.notes ? `
            <blockquote class="text-xs text-[#52525b] leading-relaxed max-h-28 overflow-y-auto scrollbar-hide my-2.5 bg-[#fbfbfa] p-3 rounded-lg border-l-2 border-[#18181b] font-normal">
              ${item.notes}
            </blockquote>
          ` : ''}
...
function openEditModal(itemId) {
  ...
  if (!item) {
    window.showToast('This item is no longer available. Refresh and try again.', 'error');
    return;
  }
...
async function handleSaveContent(e) {
  ...
  if (!title) {
    window.showToast('A title is required.', 'error');
